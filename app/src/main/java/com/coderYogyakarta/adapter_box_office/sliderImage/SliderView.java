...
public class SliderView extends ViewPager {
    ...
    public void setDurationScroll(int milis){
        try {
            Class<?> viewPager=ViewPager.class;
            Field scroller=viewPager.getDeclaredField("mScroller");
            scroller.setAccessible(true);
            scroller.set(this,new OwnScroller(getContext(),milis));
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public class OwnScroller extends Scroller{
        ...
        private int durationScrollMilis=1;

        public OwnScroller(Context context,int durationScrollMilis) {
            super(context,new DecelerateInterpolator());
            this.durationScrollMilis=durationScrollMilis;
        }
